...
// $('#folder').on('keyup',function(e){
// 	folderValue = 

// });
function showFiles(){
	
	var showfile =  $('#folder').val();
	var array = showfile.split('/');
	if (showfile===''){
		alert('Please Enter Study folder location');
		return false;
	}
	if (array[0]==='') {
		alert('Study folder location is not Properly Set. (did you start with / ?)');
		return false;
	}
	...
	if (last===-1){
		...
		user = showfile.slice(0,last);
		study = array[length-1];
	}

	window.open(config.showFilesLink+'?user='+user+'&study='+study);
	return false;
...
function getFile(){
	...
	var filedata = '<tr><td>'+timestamp+'</td><td><a href=\''+value('folder')+'\' >'+value('folder')+' </a></td><td>'+value('researchEmail')+'</td><td>'+value('researchName')+'</td><td>'+
